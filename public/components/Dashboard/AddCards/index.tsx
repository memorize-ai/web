...
import AddCardsContext from 'context/AddCards'
...
export default () => {
	...
	const { slugId, slug, sectionId } = router.query as {
		slugId: string
		slug: string
		sectionId?: string
	}
	...
	const setSection = (section: Section | undefined) => {
		const hasSectionExtension = section && !section.isUnsectioned
		
		router.push(
			`/decks/[slugId]/[slug]/add${
				hasSectionExtension ? '/[sectionId]' : ''
			}`,
			`/decks/${
				deck?.slugId ?? ''
			}/${
				...
				hasSectionExtension ? `/${section.id}` : ''
			}`
		)
	}
