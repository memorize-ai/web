...
const getNextDeck = async () => {
  ...
  if (empty) throw new Error("Unable to find deck");
...
const getNextCard = async (deck: Deck) => {
  ...
  if (empty) throw new Error("Unable to find card");
...
export const sendNextFact = async () => {
  ...
  await Promise.all([sendFact(fact), fact.deck.updateNextPostedCard()]);
};
